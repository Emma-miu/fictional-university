...
class Search {
  ...
  getResults() {
      ...
      this.resultsDiv.html(`
        <div class="row">
          <div class="one-third">
            ...
            ${results.generalInfo.map(item => `<li><a href="${item}">${item.title}</a> ${item.postType == 'post' ? `by ${item.authorName}` : ''}</li>`).join('')}
          ...
          <div class="one-third">
            ...
            ${results.programs.map(item => `<li><a href="${item}">${item.title}</a></li>`).join('')}
          ${results.programs.length ? '</ul>' : ''}

            <h2 class="search-overlay__section-title">Professors</h2>
            ${results.professors.length ? '<ul class="professors-cards">' : `<p>No Professor match that search</p>`}
          ...
          <div class="one-third">
            ...
            ${results.campuses.length ? '<ul class="link-list min-list">' : `<p>No Campus match that search <a href="${universityData.root_url}/campuses">View all Campuses</a></p>`}
            ${results.campuses.map(item => `<li><a href="${item.permalink}">${item.title}</a></li>`).join('')}
          ${results.campuses.length ? '</ul>' : ''}

            <h2 class="search-overlay__section-title">Events</h2>
            ${results.events.length ? '' : `<p>No evente match that search <a href="${universityData.root_url}/events">View all Events</a></p>`}
    ...
    // search using the traditional rest api
    // $.when(
    ...
    //         ${combinedResults.map(item => `<li><a href="${item}">${item.title.rendered}</a> ${item.type == 'post' ? `by ${item.authorName}` : ''}</li>`).join('')}
      ...
      <div class="search-overlay">
        <div class="search-overlay__top">
          <div class="container">
            ...
            <input class="search-term" placeholder="What are you looking for?" type="text" name="" id="search-term" autocomplete="off">
